// 搜尋建議下拉選單功能（分怪物、裝備、其他）
let suggestionIndex = -1
const searchInput = document.getElementById('search')
const suggestionBox = document.getElementById('search-suggestions')

function getSuggestionGroups(keyword) {
  if (!dropData || !nameToIdMap) return { monsters: [], equips: [], others: [] };
  const lower = keyword.toLowerCase();
  // 怪物
  const monsters = Object.keys(dropData).filter(name => name.toLowerCase().includes(lower));
  // 掉落物分類
  const equips = [];
  const others = [];
  const seen = new Set();
  Object.values(dropData).forEach(items => {
	items.forEach(item => {
	  if (!item.toLowerCase().includes(lower)) return;
	  if (seen.has(item)) return;
	  seen.add(item);
	  const id = parseInt(nameToIdMap[item] || '0');
	  if ((id >= 1000001 && id <= 1999999) || (id >= 2060000 && id <= 2079999) || (id >= 2330000 && id <= 2339999)) {
		equips.push(item);
	  } else {
		others.push(item);
	  }
  ...
  return { monsters, equips, others };
}

function renderSuggestionList(groups) {
  let html = '';
  let idx = 0;
  const sections = [];
  if (groups.monsters.length) {
	sections.push(`<div class="suggestion-section">怪物</div>` +
  ...
  if (groups.equips.length) {
	sections.push(`<div class="suggestion-section">裝備</div>` +
  ...
  if (groups.others.length) {
	sections.push(`<div class="suggestion-section">其他</div>` +
...
function getFlatList(groups) {
  // 依照顯示順序展平成一個陣列
  return [
	...groups.monsters.map(name => ({ name, type: 'monster' })),
	...groups.equips.map(name => ({ name, type: 'equip' })),
	...groups.others.map(name => ({ name, type: 'other' })),
  ];
}
...
function showSuggestions(keyword) {
  ...
  const groups = getSuggestionGroups(keyword);
  const flatList = getFlatList(groups);
  if (flatList.length === 0) {
	suggestionBox.style.display = 'none';
	return;
  }
  suggestionBox.innerHTML = renderSuggestionList(groups);
...
searchInput.addEventListener('keydown', function (e) {
  ...
  if (e.key === 'ArrowDown') {
	...
	if (suggestionIndex >= 0 && items[suggestionIndex]) {
	  this.value = items[suggestionIndex].dataset.value;
	  suggestionBox.style.display = 'none';
	  refresh();
	  e.preventDefault();
	}
